...
class TestFormatUrlLine:
    ...
    def test_quality_newline_rejected(self):
...
class TestValidateTargets:
    ...
    def test_newline_url_rejected(self):
        with pytest.raises(ValueError):
            validate_room_target("https://live.douyin.com/1\n# x", None)


class TestUpdateConfigLine:
    def test_update_preserves_comments_and_cleans_tmp(self, tmp_path):
...
class TestVerifyWebPassword:
    ...
    def test_malformed_iterations_returns_false(self):
...
class TestReadWebConfig:
    def test_defaults_include_trusted_proxy(self, tmp_path):
        cfg = tmp_path / "config.ini"
