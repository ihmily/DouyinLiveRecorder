...
跨平台 FFmpeg 自动检测与安装工具，支持 Windows/macOS/Linux。

主要功能:
- 自动检测 FFmpeg 是否已安装
- Windows: 优先官方源 (gyan.dev)，备用蓝奏云
- macOS: 使用 Homebrew 安装
- Linux: 自动识别 yum/apt 包管理器
- 自动更新 PATH 环境变量
- 下载进度条显示
"""
...
execute_dir = os.path.split(os.path.realpath(sys.argv[0]))[0]  # 脚本执行目录
current_env_path = os.environ.get('PATH')  # 保存原始 PATH
ffmpeg_path = os.path.join(execute_dir, 'ffmpeg')  # FFmpeg 安装目录
...
def install_ffmpeg_windows() -> bool:
    """Windows FFmpeg 安装（主入口：官方源优先，蓝奏云备用）"""
...
def install_ffmpeg_linux():
    """Linux: 自动选择 yum (RHEL/CentOS) 或 apt (Debian/Ubuntu) 安装"""
    is_RHS = True  # RedHat 系标志
