...
- **`real_url` 为空必须跳过录制链**：`main.py` 录制执行链（文件名构造/ffmpeg 命令组装）位于 `if real_url:` 构建块**之外**，但依赖块内赋值的 `title_in_name`/`ffmpeg_command`。`select_source_url` 返回 None 时若不拦截，会在 TS 分支触发 `UnboundLocalError: title_in_name`（斗鱼 hw CDN 全候选探针 405/403 时实测），或复用上一轮残留命令。必须在选源后 `if not real_url: 告警+等待+continue`。
