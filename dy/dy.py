...
def WebcastProductChangeMessage(data):
    ...
    data = json_format.MessageToDict(commonTextMessage, preserving_proto_field_name=True)
    log = json.dumps(data, ensure_ascii=False)
    # print('[WebcastProductChangeMessage] [] [房间Id：' + liveRoomId + '] ｜ ' + log)
...
def unPackWebcastChatMessage(data):
    ...
    if record_save_filepath is not None:
        log = f"[live_url:{live_url}] {content}"
...
def onClose(ws, a, b):
    print('[onClose] [webSocket Close事件] [房间Id：' + liveRoomId + ']')


def onOpen(ws):
    _thread.start_new_thread(ping, (ws,))
    print('[onOpen] [webSocket Open事件] [房间Id：' + liveRoomId + ']')
...
def ping(ws):
    while True:
        ...
        data = obj.SerializeToString()
        ws.send(data, websocket.ABNF.OPCODE_BINARY)
        # print('[ping] [💗发送ping心跳] [房间Id：' + liveRoomId + '] ====> 房间🏖标题【' + liveRoomTitle + '】')
        time.sleep(10)
