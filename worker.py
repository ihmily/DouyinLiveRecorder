...
class Platform(Base):
    ...
    __table_args__ = (
        UniqueConstraint("name", name="platforms_name_key"),
    )
...
class WorkerNode(Base):
    ...
    __table_args__ = (
        UniqueConstraint("nodeId", name="worker_nodes_nodeId_key"),
    )


class LivestreamWorkerAssignment(Base):
    __tablename__ = "livestream_worker_assignments"

    id = mapped_column(String, primary_key=True, nullable=False)
    livestreamId = mapped_column(
        String,
        ForeignKey("live_streams.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False
    )
    workerNodeId = mapped_column(
        String,
        ForeignKey("worker_nodes.id", onupdate="CASCADE", ondelete="CASCADE"),
        nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint("livestreamId", "workerNodeId",
                         name="livestream_worker_assignments_livestreamId_workerNodeId_key"),
    )
...
def sync_urls(ini_URL_content, db_urls, url_config_file):
    ini_URL_content = ini_URL_content.strip()
    # 提取 ini_URL_content 中的 URL 和完整行
    ini_url_to_line = {}
    if ini_URL_content:
        for line in ini_URL_content.strip().split('\n'):
            ...
            ini_url_to_line[url] = line

    ini_urls = set(ini_url_to_line.keys())

    # 提取 db_urls 中的 URL 列表
    db_urls_set = set(db_urls)

    urls_to_remove = ini_urls - db_urls_set

    # 找出需要添加的 URL（db_urls 中有但 ini_URL_content 中没有的）
    urls_to_add = db_urls_set - ini_urls
    ...
    for url in ini_urls:
        if url in db_urls_set:
            updated_lines.append(ini_url_to_line[url])

    # 添加需要的 URL（只添加 URL）
    for url in urls_to_add:
        updated_lines.append('高清,' + url)
        print("新增直播链接：" + url)

    for url in urls_to_remove:
        print("删除直播链接：" + url)

    # 将更新后的 URL 列表写回 url_config_file
...
engine = create_engine(
    read_config_value(config, '工作节点设置', 'db_url', "sqlite:///.db"),
    echo=False)
...
signal.signal(signal.SIGTERM, sys_quit)  # 捕获 kill 命令
...
while True:
    ...
    try:
        ...
        with Session() as session:
            # 获取需要监控的直播
            db_lives = session.execute(select(LivestreamWorkerAssignment).where(
                LivestreamWorkerAssignment.workerNodeId == device_id,
                LivestreamWorkerAssignment.referenceCount >= 1,
            )).scalars().all()
            db_urls = []
            for db_live in db_lives:
                db_live_stream = session.execute(select(LiveStream).where(
                    LiveStream.id == db_live.livestreamId,
                )).scalars().first()
                if db_live_stream:
